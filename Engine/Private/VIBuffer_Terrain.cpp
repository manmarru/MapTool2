...
HRESULT CVIBuffer_Terrain::Initialize_Prototype(int iNumVerticesX, int iNumVerticesZ)
{
	_ulong			dwByte = {};

	m_iNumVerticesX = max(1, iNumVerticesX);
	m_iNumVerticesZ = max(1, iNumVerticesZ);
...
#pragma region VERTEX_BUFFER
	...
	for (size_t i = 0; i < m_iNumVerticesZ; i++)
	{
		for (size_t j = 0; j < m_iNumVerticesX; j++)
		{
			...
			m_pVerticesPos[iIndex] = _float3(j, 0, i);
...
void CVIBuffer_Terrain::Resize(int iSizeX, int iSizeZ)
{
	Safe_Release(m_pVB);
	Safe_Release(m_pIB);
	Initialize_Prototype(iSizeX, iSizeZ);
}
...
CVIBuffer_Terrain * CVIBuffer_Terrain::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, _int iX, _int iZ)
{
